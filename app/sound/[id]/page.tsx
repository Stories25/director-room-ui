    ...
    <main className="flex h-screen w-screen flex-col overflow-hidden" style={{ background: 'var(--canvas)' }}>
      ...
      <div className="flex-1 overflow-y-auto w-full">
        <div style={{ maxWidth: 760, margin: '0 auto', paddingTop: 36, paddingBottom: 100, paddingLeft: 32, paddingRight: 32 }}>
          ...
          {/* Selected track banner + playback — unified block */}
          <div className="mb-8">
            {/* Track info */}
            <div className="flex items-start justify-between gap-4 mb-5">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-1.5">
                  <span className="text-sm font-semibold" style={{ color: GOLD_BRIGHT }}>
              ...
              <div className="text-right flex-none pt-0.5">
                <p className="text-[10px] font-slate" style={{ color: 'var(--text-muted)' }}>
                  {activeBgm.timestamps.length} shots · {fmt(durS)}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              {/* Play / Pause — minimal, no border box */}
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.background = `rgba(200,146,28,0.28)` }}
                onMouseLeave={e => { e.currentTarget.style.background = GOLD_DIM }}
              >
                {playing
                  ? <Pause className="w-3.5 h-3.5" style={{ color: GOLD_BRIGHT }} />
                  : <Play  className="w-3.5 h-3.5 ml-0.5" style={{ color: GOLD_BRIGHT }} />
                }
              </button>
              ...
              <div className="flex-1 flex items-center gap-3">
                <div
                  className="flex-1 relative h-[3px] rounded-full cursor-pointer"
                ...
                <span className="flex-none text-[10px] font-slate tabular-nums" style={{ color: 'var(--text-muted)' }}>
                  {fmt(time)} / {fmt(durS)}
                </span>
              </div>
            </div>
